...
05_evaluate_mesh.py
-------------------
Schritt 5 der Pipeline: Geometrische Evaluation der rekonstruierten Meshes.

Eingabe:  data/processed/preprocessed.npy  (Originalpunktwolke)
          data/processed/darmstadt_pred.npy (Segmentierungsergebnis mit Labels)
          data/output/*_LOD0/LOD1/LOD2.ply  (rekonstruierte Meshes)

Ausgabe:  Konsolenausgabe mit Evaluationsmetriken

Metriken pro Klasse:
  - Vollständigkeit: Anteil der Eingabepunkte deren Abstand zum Mesh < SCHWELLENWERT
  - Mittlerer Abstand: Durchschnittliche Entfernung Punkt -> nächster Mesh-Vertex
  - Maximaler Abstand: Worst-Case-Abweichung (aehnlich einseitiger Hausdorff-Distanz)
  - LOD-Dreiecksanzahl: Polygonanzahl aller drei LOD-Stufen

Hinweis: Da kein unabhaengiges Referenzmodell vorliegt, wird die Vollständigkeit
gegen die eigene Eingabepunktwolke gemessen, nicht gegen ein Ground-Truth-Mesh.
"""
...
SCHWELLENWERT = 0.5  # Vollständigkeitsschwelle in Metern
...
# Daten laden 
# Originalpunktwolke (XYZ) und Segmentierungslabels werden geladen
# Die Evaluation misst wie gut das rekonstruirte Mesh die Eingabepunkte abdeckt
print("=" * 60)
...
# Für jede Klasse wird das LOD0-Mesh geladen und gegen die Eingabepunkte gemessen
# Vollständigkeit: Anteil der Punkte die innerhalb SCHWELLENWERT vom Mesh liegen
# Mittlerer Abstand: Durchschnittliche Entfernung von Punkt zu nächstem Mesh-Vertex
# Maximaler Abstand: Maximale Abweichung (Randatefakte koennen diesen Wert stark erhöhen)
...
for klasse_id, name in KLASSEN.items():
    ...
    # LOD0-Mesh laden (wird fuer Abstandsberechnung genutzt)
    ...
    # Klassenpunkte als Open3D PointCloud
    pcd_klasse = o3d.geometry.PointCloud()
    pcd_klasse.points = o3d.utility.Vector3dVector(xyz[maske])

    # Mesh-Vertices als Vergleichspunktwolke
    # Abstand: für jeden Eingabepunkt -> ncähster Mesh-Vertex
    ...
    # Dreiecksanzahl aller drei LOD-Stufen einlesen
...
if ergebnisse:
    ...
    print(f"\nMittlere Vollstädigkeit (<{SCHWELLENWERT}m): {gesamt_vol:.1f}%")
...
print(f"\nGesamtgröße LOD0 (alle Klassen): {gesamt_groesse:.1f} MB")
